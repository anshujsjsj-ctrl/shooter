...
import requests , json , binascii , time , urllib3 , base64 , datetime , re ,socket , threading , random , os , asyncio , aiohttp , ssl
...
# --- FRIEND REQUEST LOGIC START ---

# ==========================================
#      FRIEND ACTION API (UPDATED)
# ==========================================
# --- OPEN xC4.py and add/replace this function at the end ---

def Friend_Action_API_Remote(uid, action, jwt_token):
    # Your deployed API URL
    url = "https://bd-req-spam-api.vercel.app/"
    
    # Parameters: uid, action, and the BOT's token
    params = {
        "uid": uid,
        "action": action,
        "token": jwt_token
    ...
    try:
        ...
        # The external API expects parameters in the URL (GET request) based on your provided code
        ...
        try:
            ...
            # If API returns non-JSON (like 500 error page)
            return {"success": False, "message": f"API Error: Status {response.status_code}"}
            
    except requests.exceptions.RequestException as e:
        return {"success": False, "message": "Connection Failed or Timeout"}
        
        
# ==========================================
#      GUILD JOIN API (UPDATED)
# ==========================================
def Guild_Join_API(guild_id, jwt_token):
    # Aapki API URL
    url = f"https://apijoin-clan.vercel.app/join?clan_id={guild_id}&jwt={jwt_token}"
    
    try:
        # Timeout 10 seconds taaki slow network pe crash na ho
        response = requests.get(url, timeout=10)
        
        # Koshish karein JSON read karne ki, chahe status code 200 ho ya 400
        try:
            return response.json()
        except ValueError:
            # Agar API ne JSON nahi bheja (Server Error/HTML page)
            return {"error": f"Invalid API Response (Status: {response.status_code})"}
            
    except requests.exceptions.RequestException as e:
        return {"error": "Connection Failed/Timeout"}
        
async def Room_Spam(Uid , Rm , Nm , K , V):
    # Badge value can be customized
    same_value = 32768 
    fields = {1: 78, 2: {1: int(Rm), 2: f"[{ArA_CoLor()}]{Nm}", 3: {2: 1, 3: 1}, 4: 330, 5: 1, 6: 201, 10: await xBunnEr(), 11: int(Uid), 12: 1, 15: {1: 1, 2: same_value}, 16: same_value, 31: {1: 1, 2: same_value}, 32: same_value}}
    return await GeneRaTePk((await CrEaTe_ProTo(fields)).hex() , '0e15' , K , V)
    
# --- START OF FILE xC4.py (Add at the bottom) ---

# --- START OF FILE xC4.py (Sabse neeche add karein) ---

async def Start_Game_Packet(K, V, region):
    # 1: 6 ka matlab "Start Game Request" hota hai
    fields = {1: 6}
    
    # Region ke hisab se Header set karna zaroori hai
    if region.lower() == "ind":
        packet = '0514'
    elif region.lower() == "bd":
        packet = "0519"
